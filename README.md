...
- Upgrade ml-agent and ray to recent versions
  - The requirements after the first block (splitted by an empty line) of ```requirements.txt``` is generated by pip, so need to be updated according to the update of first block by ```pip3 freeze > requirements-new.txt```, and copy the lines in ```requirements-new.txt``` back to ```requirements.txt``` as the second block.
